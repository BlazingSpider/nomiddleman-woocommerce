...
class NMM_Gateway extends WC_Payment_Gateway {
    ...
    private static $NMMTime = null;
    private static $NMMRates = null;
    ...
    public function validate_fields() {
        ...
        if (WC()->session->get('chosen_payment_method') === $this->id) {
            try {
                ...
                $this->get_crypto_value();
                var_dump($this->id);
                var_dump(self::$NMMRates);
                if (self::$NMMRates == null || empty(self::$NMMRates['BTC']) || empty(self::$NMMRates['ETH']) || empty(self::$NMMRates['USDT'])) {
                    NMM_Util::log(__FILE__, __LINE__, "crypto quote request not right");
                    wc_add_notice('Not able to get exchange rates ', 'error');
                }
            }
            catch ( \Exception $e) {
    ...
    public function thank_you_page($order_id) {
        ...
        try {
            ...
            $this->get_crypto_value();
            $cryptoPerCad = self::$NMMRates[$cryptoId];
            ...
            $cryptoTotalPreMarkup = round($cadTotal / $cryptoPerCad, $crypto->get_round_precision(), PHP_ROUND_HALF_UP);
            $cryptoTotal = $cryptoTotalPreMarkup * $cryptoPriceRatio;

            $dustAmount = apply_filters('nmm_dust_amount', 0.000000000000000000, $cryptoId, $cryptoPerCad, $crypto->get_round_precision(), $cadTotal, $cryptoTotal);
    ...
    private function get_crypto_value() {
        if (self::$NMMTime == null || time() - self::$NMMTime > 40) {
            self::$NMMTime = time();
            $api_endpoint = 'https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest';
	        $api_args     = array( 'headers' => array( 'X-CMC_PRO_API_KEY' => 'a56a1012-1c60-4914-800a-6e7984c76292' ) );
	        $queries      = array(
                'id'   => '1,1027,825',
                'convert' => 'CAD',
            );
            ...
            if ( is_wp_error( $response ) ) {
                ...
                $data = json_decode($body);
                    self::$NMMRates = array(
                    'BTC' => $data->data->{'1'}->{'quote'}->{'CAD'}->{'price'},
                    'ETH' => $data->data->{'1027'}->{'quote'}->{'CAD'}->{'price'},
                    'USDT' => $data->data->{'825'}->{'quote'}->{'CAD'}->{'price'}
                );
            }
        }
    }
